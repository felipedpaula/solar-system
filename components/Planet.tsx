...
const getRingRadii = (data: CelestialBodyData) => {
    const innerMultiplier = data.ringInnerRadiusMultiplier ?? 1.4;
    const outerMultiplier = data.ringOuterRadiusMultiplier ?? 2.3;
    return {
        innerRadius: data.radius * innerMultiplier,
        outerRadius: data.radius * outerMultiplier
    };
};
...
const TexturedRingMesh: React.FC<{ data: CelestialBodyData }> = ({ data }) => {
    ...
    const ringGeometry = useMemo(() => {
        const { innerRadius, outerRadius } = getRingRadii(data);
...
const Planet: React.FC<PlanetProps> = ({ data, isPaused, onClick, initialAngle, frozenPosition, selectedBodyId, onRegisterMoonMesh }) => {
    ...
    useFrame((state) => {
        ...
        if (groupRef.current && !isPaused) {
            ...
            let y = 0;
            let z = Math.sin(currentAngle) * data.distance;

            if (data.orbitalInclination) {
                const inclinationRad = THREE.MathUtils.degToRad(data.orbitalInclination);
                y = Math.sin(currentAngle) * data.distance * Math.sin(inclinationRad);
                z = Math.sin(currentAngle) * data.distance * Math.cos(inclinationRad);
            }
